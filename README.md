...
Tài liệu dưới đây được dịch từ.
...
## Configuration
* Đặt các đoạn code khởi tạo chương trình trong file config/initializers. Nó sẽ được thực thi khi mà ứng dụng khởi động
* Thiết lập cấu hình cho môi trường development, staging, production trong các file tương ứng dưới folder config/environments/ (ex: config/environments/production.rb)
* Thiết lập các cấu hình được sử dụng chung cho tất cả các environment trong file config/application.rb
## Routing
* When you need to add more actions to a RESTful resource (do you
  really need them at all?) use `member` and `collection` routes.
* Việc sử dụng các convention dưới đây trong file routes.rb sẽ giúp việc quản lý các URL,cũng như tổ chức kiến trúc vật lý project của bạn được trong sáng, rõ ràng và dễ bảo trì hơn.
* Bất cứ khi nào muốn add thêm một action vào controller sử dụng kiến trúc REST, bạn nên khai báo trong resource.
...
* Nếu cần định nghĩa nhiều action trong member/collection thì cú pháp sẽ là:
...
* Use nested routes to express better the relationship between
  ActiveRecord models.
* Sử dụng resource lồng nhau để diễn đạt tốt hơn mối quan hệ giữa các ActiveRecord model
...
* Sử dụng namespace để nhóm các action có mối quan hệ với nhau thành Group
...
* Không bao giờ khai báo những route mà có thể gây nhập nhằng dẫn đến trường hợp các request khác nhau 
có thể ánh xạ đến cùng một action trong cùng một controller
...
* Giữ controller của bạn đảm nhiệm đúng chức năng của nó là "điều hướng", không nên đặt những xử lý logic vào trong controller thay vào đó hãy đặt nó trong model tương ướng. 
...
* If you need model objects that support ActiveRecord behavior(like
  validation) use the
* Tên của model phải ngắn gọn, có nghĩa nhưng không được viết tắt
* Nếu cần non-ActiveRecord model objects hỗ trợ một vài thuộc tính của ActiveRecord(giống validation) sử dụng 
...
* Avoid altering ActiveRecord defaults (table names, primary key, etc)
  ...
  your control).
* Tránh thay đổi các giá trị mặc định của ActiveRecord (tên bảng, khóa chính, ...) trừ phi bạn có những lý do chính đáng (ví dụ như database không còn dưới sự kiểm soát của bạn)

    ```Ruby
    # bad - don't do this if you can modify
    # bad - bạn không nên sửa schema file giống như dưới đây	
...
* Nhóm các methods macro (has_many, validates,..) vào phần trên cùng trong class được khai báo

    ```Ruby
    class User < ActiveRecord::Base
      # keep the default scope first (if any)
      # khai báo scope macro trước
      default_scope { where(active: true) }

      # constants come up next
      # Tiếp theo là khai báo hằng
      GENDERS = %w(male female)

      # afterwards we put attr related macros
      # Sau đó là các khai báo thuộc tính cho lớp
      attr_accessor :formatted_date_of_birth

      attr_accessible :login, :first_name, :last_name, :email, :password

      # followed by association macros
      # theo sau là các association
      belongs_to :country
      ...
      # tiếp đến chúng ta gọi các callbacks
      before_save :cook
      before_save :update_username_lower

      # other macros (like devise's) should be placed after the callbacks
      # cuối cùng các macros khác nên được đặt sau các callback
...
* Nên sử dụng 'has_many :through' thay vì 'has_and_belongs_to_many'. Vì sử dụng 'has_many :through' cho phép add thêm các thuộc tính và các validations vào join model.
    ...
    # cách tốt hơn- sử dụng has_many :through
    class User < ActiveRecord::Base
...
* Nên sử dụng 'self[:attribute]' thay vì  'read_attribute(:attribute)'
...
* khi mà bạn muốn tự viết một validation được sử dụng nhiều lần hãy tạo ra một file riêng để lưu trữ các validation này
...
* Đặt các validators của bạn trong thư mục 'lib/validators'
* Đặt tên cho các scope ngắn ngọn, dễ hiểu.
...
* Dùng 'lambdas' để khởi tạo 'lười' các scope
...
scopes like this.
* Khi một scope được định nghĩa bởi lambda và các tham số,là phức tạp, bạn nên tạo riêng một class method, method này đóng vai trò như scope và trả ra môt đối tượng thuộc lớp `ActiveRecord::Relation`.
